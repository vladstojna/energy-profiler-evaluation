...
public:
  ...
  timer(duration &dur) : _dur(dur), _start(time_point::clock::now()) {}
...
void generate(std::vector<std::mt19937_64> &engines, std::size_t count,
...
std::size_t find_rng_count(std::vector<std::mt19937_64> &engines,
                           const std::chrono::nanoseconds &period,
                           double tolerance) {
  using nanos = std::chrono::nanoseconds;
  auto is_accurate = [](const nanos &dur, const nanos &period,
                        double tolerance) {
    return std::abs((dur - period).count()) <=
           tolerance *
               std::chrono::duration_cast<decltype(dur - period)>(period)
                   .count();
  };
  ...
  for (std::size_t i = 0, count = start; i < max_iters; i++) {
    ...
    if (is_accurate(dur, period, tolerance))
      return count;
    auto ratio =
        std::chrono::duration_cast<std::chrono::duration<double>>(period) / dur;
    count *= ratio;
  };
  throw std::runtime_error(
      "Reached max iterations without satisfying accuracy");
}

__attribute__((noinline)) void do_work(std::vector<std::mt19937_64> &engines,
                                       const std::chrono::nanoseconds &period,
                                       std::int32_t count,
