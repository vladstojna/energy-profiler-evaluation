...
namespace detail {
template <typename T> struct int_distribution_traits {
  using value_type = T;
...
template <typename T> struct real_distribution_traits {
  using value_type = T;
...
struct host_placement {
  template <typename T> using container = thrust::host_vector<T>;
  static const decltype(thrust::host) policy;
};

struct device_placement {
  template <typename T> using container = thrust::device_vector<T>;
...
template <typename> struct distribution_traits {};
...
template <>
struct distribution_traits<float> : detail::real_distribution_traits<float> {};

template <>
struct distribution_traits<double> : detail::real_distribution_traits<double> {
};

template <typename T> struct element_traits {
  using value_type = T;
...
struct greater_than_threshold {
  __host__ __device__ bool operator()(const std::int32_t &x) {
    return call_impl(x);
  }
  __host__ __device__ bool operator()(const std::int64_t &x) {
    return call_impl(x);
  }
...
private:
  template <typename T> __host__ __device__ bool call_impl(const T &x) {
...
template <typename T>
NO_INLINE thrust::host_vector<T> random_vector(std::size_t n,
                                               std::mt19937_64 &engine) {
  using traits = element_traits<T>;
  tp::sampler smp(g_tpr);
  (void)smp;
  typename traits::dist_traits::distribution dist{traits::dist_traits::lower,
                                                  traits::dist_traits::upper};
...
NO_INLINE typename BinaryOp::value_type reduce_work(
  ...
  for (std::size_t i = 0; i < iters; i++)
    results[i] =
        thrust::reduce(Placement::policy, std::begin(vec), std::end(vec),
                       BinaryOp::init_value, typename BinaryOp::functor{});
  return results.front();
}

template <typename Placement, typename T>
NO_INLINE T count_if_work(const typename Placement::container<T> &vec,
                          std::size_t iters) {
  tp::sampler smp(g_tpr);
  (void)smp;
  thrust::host_vector<T> results(iters);
  for (std::size_t i = 0; i < iters; i++)
    results[i] = thrust::count_if(Placement::policy, std::begin(vec),
                                  std::end(vec), greater_than_threshold{});
  return results.front();
}

template <typename Placement, typename T>
NO_INLINE T max_element_work(const typename Placement::container<T> &vec,
                             std::size_t iters) {
  tp::sampler smp(g_tpr);
  (void)smp;
  thrust::host_vector<T> results(iters);
  for (std::size_t i = 0; i < iters; i++)
    results[i] =
        *thrust::max_element(Placement::policy, std::begin(vec), std::end(vec));
  return results.front();
}

template <typename Placement, typename T>
NO_INLINE T min_element_work(const typename Placement::container<T> &vec,
                             std::size_t iters) {
  tp::sampler smp(g_tpr);
  (void)smp;
  thrust::host_vector<T> results(iters);
  for (std::size_t i = 0; i < iters; i++)
    results[i] =
        *thrust::min_element(Placement::policy, std::begin(vec), std::end(vec));
  return results.front();
}

template <typename Placement, typename T>
NO_INLINE T inner_product_work(const typename Placement::container<T> &vec,
                               std::size_t iters) {
  tp::sampler smp(g_tpr);
  (void)smp;
  thrust::host_vector<T> results(iters);
  for (std::size_t i = 0; i < iters; i++)
    results[i] =
        thrust::inner_product(Placement::policy, vec.begin(), vec.end(),
                              vec.begin(), sum<T>::init_value);
  return results.front();
}

template <typename BinaryOp>
NO_INLINE typename BinaryOp::value_type
reduce_host(std::size_t n, std::size_t iters, std::mt19937_64 &engine) {
...
template <typename BinaryOp>
NO_INLINE typename BinaryOp::value_type
reduce_device(std::size_t n, std::size_t iters, std::mt19937_64 &engine) {
  auto vec = random_vector<typename BinaryOp::value_type>(n, engine);
...
template <typename T>
NO_INLINE T count_if_host(std::size_t n, std::size_t iters,
                          std::mt19937_64 &engine) {
...
template <typename T>
NO_INLINE T count_if_device(std::size_t n, std::size_t iters,
                            std::mt19937_64 &engine) {
  auto vec = random_vector<T>(n, engine);
...
template <typename T>
NO_INLINE T max_element_host(std::size_t n, std::size_t iters,
                             std::mt19937_64 &engine) {
  auto vec = random_vector<T>(n, engine);
...
template <typename T>
NO_INLINE T max_element_device(std::size_t n, std::size_t iters,
                               std::mt19937_64 &engine) {
  auto vec = random_vector<T>(n, engine);
  thrust::device_vector<T> d_vec = vec;
...
template <typename T>
NO_INLINE T min_element_host(std::size_t n, std::size_t iters,
                             std::mt19937_64 &engine) {
  auto vec = random_vector<T>(n, engine);
...
template <typename T>
NO_INLINE T min_element_device(std::size_t n, std::size_t iters,
                               std::mt19937_64 &engine) {
  auto vec = random_vector<T>(n, engine);
  thrust::device_vector<T> d_vec = vec;
...
template <typename T>
NO_INLINE T inner_product_host(std::size_t n, std::size_t iters,
                               std::mt19937_64 &engine) {
  auto vec = random_vector<T>(n, engine);
...
template <typename T>
NO_INLINE T inner_product_device(std::size_t n, std::size_t iters,
                                 std::mt19937_64 &engine) {
  auto vec = random_vector<T>(n, engine);
  thrust::device_vector<T> d_vec = vec;
...
#define DISPATCH_REDUCE(place, type, op_type)                                  \
  do {                                                                         \
    if (args.where == placement::place && args.etype == element_type::type &&  \
        args.op == operation::op_type) {                                       \
      std::cerr << #place " " #type " " #op_type "\n";                         \
      std::cerr << reduce_##place<op_type<type##_t>>(args.n, args.iters,       \
                                                     engine)                   \
                << "\n";                                                       \
      return;                                                                  \
    }                                                                          \
...
#define DISPATCH_EXTREMA(place, type, op_type)                                 \
  do {                                                                         \
    if (args.where == placement::place && args.etype == element_type::type &&  \
        args.op == operation::op_type) {                                       \
      std::cerr << #place " " #type " " #op_type "\n";                         \
      std::cerr << op_type##_element_##place<type##_t>(args.n, args.iters,     \
                                                       engine)                 \
                << "\n";                                                       \
      return;                                                                  \
    }                                                                          \
