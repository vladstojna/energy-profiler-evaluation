...
using cublasxt_handle =
    util::unique_handle<cublasXtHandle_t, func_obj<cublasxt_destroy>>;

namespace detail {
template <typename> struct gemm_caller {};
template <> struct gemm_caller<float> : func_obj<cublasXtSgemm> {};
template <> struct gemm_caller<double> : func_obj<cublasXtDgemm> {};
...
NO_INLINE NO_CLONE void
gemm_compute(cublasxt_handle &handle, std::size_t iters, std::size_t M,
             std::size_t N, std::size_t K, const Real *alpha, const Real *a,
             const Real *b, const Real *beta, Real *c) {
  tp::sampler smp(g_tpr);
  for (decltype(iters) i = 0; i < iters; ++i) {
    auto res = gemm_caller<Real>::value(handle, CUBLAS_OP_N, CUBLAS_OP_N, M, N,
...
void gemm_impl(std::size_t M, std::size_t N, std::size_t K, std::size_t iters,
               cublasxt_handle &handle, std::mt19937_64 &engine) {
  ...
  gemm_compute(handle, iters, M, N, K, &alpha, a.data(), b.data(), &beta,
               c.data());
}
} // namespace detail

NO_INLINE void dgemm(std::size_t M, std::size_t N, std::size_t K,
                     std::size_t iters, cublasxt_handle &handle,
                     std::mt19937_64 &engine) {
  detail::gemm_impl<double>(M, N, K, iters, handle, engine);
}

NO_INLINE void sgemm(std::size_t M, std::size_t N, std::size_t K,
                     std::size_t iters, cublasxt_handle &handle,
                     std::mt19937_64 &engine) {
...
void set_first_device(cublasxt_handle &handle) {
  int device_ids[] = {0};
  if (auto res = cublasXtDeviceSelect(handle, 1, device_ids);
...
void set_block_dim(cublasxt_handle &handle, int block_dim) {
  if (auto res = cublasXtSetBlockDim(handle, block_dim);
      res != CUBLAS_STATUS_SUCCESS)
...
private:
  ...
  void print_usage(const char *prog) {
    std::cerr << "Usage: " << prog
              << " {dgemm,sgemm} <m> <n> <k> <block_dim> <iters>\n";
  }
};
} // namespace
