...
using cublas_handle =
    util::unique_handle<cublasHandle_t, func_obj<cublas_destroy>>;

namespace detail {
template <typename> struct gemm_caller {};
template <> struct gemm_caller<float> : func_obj<cublasSgemm> {};
template <> struct gemm_caller<double> : func_obj<cublasDgemm> {};
...
gemm_compute(cublas_handle &handle, std::size_t iters, std::size_t M,
...
void gemm_impl(std::size_t M, std::size_t N, std::size_t K, std::size_t iters,
               cublas_handle &handle, std::mt19937_64 &engine) {
  ...
  gemm_compute(handle, iters, M, N, K, &alpha, dev_a.get(), dev_b.get(), &beta,
               dev_c.get());
...
NO_INLINE void dgemm(std::size_t M, std::size_t N, std::size_t K,
                     std::size_t iters, cublas_handle &handle,
