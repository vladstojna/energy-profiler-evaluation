...
cusparseHandle_t cusparse_create() {
  ...
  if (status != CUSPARSE_STATUS_SUCCESS)
    throw std::runtime_error(
        get_cusparse_error("Error creating cuSPARSE", status));
  return handle;
}
...
void cusparse_destroy(cusparseHandle_t handle) {
  ...
  if (status != CUSPARSE_STATUS_SUCCESS) {
    std::cerr << "Error destroying cuSPARSE: " << cusparseGetErrorString(status)
              << "\n";
  }
}

using cusparse_handle =
    util::unique_handle<cusparseHandle_t, func_obj<cusparse_destroy>>;
...
cusparse_csr_create(std::int32_t M, std::int32_t N,
                    util::device_buffer<std::int32_t> &rows_csr,
                    util::device_buffer<std::int32_t> &cols,
                    util::device_buffer<Real> &values) {
  cusparseSpMatDescr_t mat;
  auto status =
      cusparseCreateCsr(&mat, M, N, cols.size(), rows_csr.get(), cols.get(),
                        values.get(), CUSPARSE_INDEX_32I, CUSPARSE_INDEX_32I,
                        CUSPARSE_INDEX_BASE_ZERO, cuda_data_type<Real>::value);
  if (status != CUSPARSE_STATUS_SUCCESS)
    throw std::runtime_error(
...
cusparseSpMatDescr_t cusparse_csr_create(std::int32_t M, std::int32_t N) {
  cusparseSpMatDescr_t mat;
  auto status =
      cusparseCreateCsr(&mat, M, N, 0, nullptr, nullptr, nullptr,
...
using cusparse_spgemm_descr =
    util::unique_handle<cusparseSpGEMMDescr_t,
                        func_obj<cusparse_spgemm_descr_destroy>>;
using cusparse_csr_mat_descr =
    util::unique_handle<cusparseSpMatDescr_t, func_obj<cusparse_csr_destroy>>;

util::buffer<std::int32_t> get_coo_rows(std::int32_t M, std::int32_t N,
                                        std::size_t NNZ,
  ...
  auto gen = [&]() {
    ...
    std::cerr << "Could not generate unique random number after " << max_iters
              << " tries, looking for first row number less than N\n";
...
get_coo_cols(std::int32_t N, std::mt19937_64 &engine,
  ...
  for (auto [rprev, rit, cprev, cit] =
           std::tuple{coo_rows.begin(), std::next(coo_rows.begin()),
                      indices.begin(), std::next(indices.begin())};
       rit <= coo_rows.end() && cit <= indices.end(); rit++, cit++) {
...
void coo2csr(cusparse_handle &handle, const util::buffer<std::int32_t> &coo,
             util::device_buffer<std::int32_t> &csr) {
  const util::device_buffer<std::int32_t> dcoo{coo.begin(), coo.end()};
  auto status =
      cusparseXcoo2csr(handle, dcoo.get(), dcoo.size(), csr.size() - 1,
                       csr.get(), CUSPARSE_INDEX_BASE_ZERO);
  cudaDeviceSynchronize();
  if (status != CUSPARSE_STATUS_SUCCESS)
    throw std::runtime_error(
        get_cusparse_error("Error converting COO to CSR", status));
}

template <typename Real>
typename util::buffer<Real>::size_type
spgemm_impl(std::int32_t M, std::int32_t N, std::int32_t K, std::size_t A_nnz,
            std::size_t B_nnz, cusparse_handle &handle,
            std::mt19937_64 &engine) {
  ...
  util::device_buffer<std::int32_t> da_csr_rows{static_cast<std::size_t>(M) +
                                                1};
  util::device_buffer<std::int32_t> db_csr_rows{static_cast<std::size_t>(K) +
                                                1};
  ...
  auto status = cusparseSpGEMM_workEstimation(
      handle, A_op_type, B_op_type, &alpha, A, B, &beta, C, dtype, alg,
      spgemm_desc, &buffer_size1, nullptr);
  if (status != CUSPARSE_STATUS_SUCCESS)
    throw std::runtime_error(
  ...
  status = cusparseSpGEMM_workEstimation(
      handle, A_op_type, B_op_type, &alpha, A, B, &beta, C, dtype, alg,
      spgemm_desc, &buffer_size1, dbuffer1.get());
  if (status != CUSPARSE_STATUS_SUCCESS)
    throw std::runtime_error(
  ...
  status = cusparseSpGEMM_compute(handle, A_op_type, B_op_type, &alpha, A, B,
                                  &beta, C, dtype, alg, spgemm_desc,
...
spdgemm(std::int32_t M, std::int32_t N, std::int32_t K, std::size_t A_nnz,
        std::size_t B_nnz, cusparse_handle &handle, std::mt19937_64 &engine) {
  return detail::spgemm_impl<double>(M, N, K, A_nnz, B_nnz, handle, engine);
}

__attribute__((noinline)) auto
spsgemm(std::int32_t M, std::int32_t N, std::int32_t K, std::size_t A_nnz,
        std::size_t B_nnz, cusparse_handle &handle, std::mt19937_64 &engine) {
  ...
  cmdargs(int argc, const char *const *argv) {
    ...
    if (op_type == "dgemm")
      ...
      throw std::invalid_argument(
          std::string("invalid work type: ").append(argv[1]));
    }
    assert(func);
  }
...
int main(int argc, char **argv) {
  try {
    ...
    std::cerr << "Resulting matrix: " << args.m << "x" << args.n << " with "
