...
using is_iterator_compatible =
    std::conjunction<is_random_access_iterator<InputIt>,
...
template <typename T>
void copy_impl(T *dest, const T *src, std::size_t count, cudaMemcpyKind kind,
               std::string_view msg) {
...
public:
  ...
  device_buffer(const device_buffer &other) : device_buffer(other.size()) {
  ...
  template <typename InputIt,
            typename = std::enable_if_t<
                detail::is_iterator_compatible<InputIt, value_type>::value>>
...
device_buffer(Iter, Iter)
    -> device_buffer<typename std::iterator_traits<Iter>::value_type>;

template <typename T> struct device_buffer_view {
  using value_type = T;
  ...
  explicit device_buffer_view() noexcept : device_buffer_view{nullptr, 0} {}
...
template <typename InputIt, typename T,
          typename = std::enable_if_t<
              detail::is_iterator_compatible<InputIt, T>::value>>
void copy(InputIt start, InputIt end, device_buffer<T> &into) {
  copy_impl(into.get(), &*start, std::distance(start, end),
...
template <typename InputIt, typename T,
          typename = std::enable_if_t<
              detail::is_iterator_compatible<InputIt, T>::value>>
void copy(const device_buffer<T> &from,
          typename device_buffer<T>::size_type count, InputIt into) {
...
template <typename T> void zero(device_buffer<T> &x) {
