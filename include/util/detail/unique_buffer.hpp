...
public:
  ...
  template <bool Const> struct iterator_impl {
    ...
    iterator_impl(pointer ptr) : _ptr(ptr) {}

    template <bool OtherConst, bool ThisConst = Const,
              std::enable_if_t<!OtherConst && ThisConst, bool> = true>
    iterator_impl(iterator_impl<OtherConst> iter) : iterator_impl(&*iter) {}
    ...
    friend difference_type operator-(const iterator_impl lhs,
  ...
  unique_buffer() noexcept : _data(), _size{} {}

  unique_buffer(pointer p, size_type size) noexcept : _data(p), _size(size) {}
