...
void curand_set_generator_seed(curandGenerator_t gen, std::random_device &rd) {
  auto status = curandSetPseudoRandomGeneratorSeed(
      gen, static_cast<std::uint64_t>(rd()) << 32 | rd());
  if (status != CURAND_STATUS_SUCCESS)
...
using generator_handle =
    util::unique_handle<curandGenerator_t, func_obj<curand_generator_destroy>>;

namespace detail {
template <typename T> struct generate_func {};

template <> struct generate_func<double> {
  auto operator()(curandGenerator_t gen, double *buff, std::size_t count) {
    return curandGenerateUniformDouble(gen, buff, count);
  }
};

template <> struct generate_func<float> {
...
__attribute__((noinline)) typename util::buffer<RealType>::size_type
generate_host(std::size_t count, std::size_t iters,
              std::vector<std::mt19937> &engines) {
  assert(count > 0);
...
template <typename RealType>
__attribute__((noinline)) typename util::device_buffer<RealType>::size_type
...
struct cmdargs {
  ...
  cmdargs(int argc, const char *const *argv)
      : count(0), iters(1), wtype(work_type::none) {
    if (argc < 4) {
      usage(argv[0]);
